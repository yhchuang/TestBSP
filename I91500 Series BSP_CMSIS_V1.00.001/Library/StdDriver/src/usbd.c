 ...
 * $Revision: 21 $
 * $Date: 20/11/24 3:34p $
 * @brief    I91500 series USBD driver source file
 *
 * @note
...
void USBD_Start(void)
{
    ...
    USBD->ATTR |= (USBD_ATTR_PHYEN_Msk|USBD_ATTR_PWRDN_Msk);
...
void USBD_CtrlOut(void)
{
    ...
    if(g_usbd_CtrlOutSize < g_usbd_CtrlOutSizeLimit)
    {
        ...
        if((g_usbd_CtrlOutSizeLimit - g_usbd_CtrlOutSize) < u32Size)
            USBD_MemCopy((uint8_t *)g_usbd_CtrlOutPointer, (uint8_t *)USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP1), (g_usbd_CtrlOutSizeLimit - g_usbd_CtrlOutSize));
        else
