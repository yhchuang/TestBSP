...
{
    return FMC_Proc(FMC_ISPCMD_WRITE, u32Addr, u32Addr + 4, &u32Data);
}
...
{
    FMC_Proc(FMC_ISPCMD_WRITE, addr_start, addr_end, data);
    return;
}
...
void UpdateConfig(unsigned int *data, unsigned int *res)
{
    ...
    FMC_Proc(FMC_ISPCMD_WRITE, Config0, Config0 + u32Size, data);
