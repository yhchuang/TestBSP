...
// I2S_SLAVE ===================================================================================================
#define I2S_SLAVE_PIN_MASK     (SYS_GPA_MFP_PA0MFP_Msk|SYS_GPA_MFP_PA1MFP_Msk|SYS_GPA_MFP_PA2MFP_Msk|SYS_GPA_MFP_PA3MFP_Msk|SYS_GPA_MFP_PA4MFP_Msk)
#define I2S_SLAVE_PIN          (SYS_GPA_MFP_PA0MFP_I2S0_MCLK|SYS_GPA_MFP_PA1MFP_I2S0_LRCK|SYS_GPA_MFP_PA2MFP_I2S0_BCLK|SYS_GPA_MFP_PA3MFP_I2S0_DO|SYS_GPA_MFP_PA4MFP_I2S0_DI)
...
{
	// Set I2S MFP
	SYS->GPA_MFP = (SYS->GPA_MFP&~I2S_SLAVE_PIN_MASK)|I2S_SLAVE_PIN;
	
	// Enable I2S0 clock.
	CLK_EnableModuleClock(I2S_MODULE);
	// Select I2S0 clock.
	CLK_SetModuleClock(I2S_MODULE, CLK_CLKSEL1_I2SSEL_HIRC, NULL);
	// I2S IPReset.
	SYS_ResetModule(I2S_RST);	
	// Open I2S0 hardware IP
	I2S_Open(I2S, I2S_SLAVE, 0, I2S_DATABIT_32, I2S_STEREO, I2S_FORMAT_I2S);
	// I2S0 Configuration
	I2S_SET_PCMSYNC(I2S, I2S_PCMSYNC_BCLK);
	I2S_SET_MONO_RX_CHANNEL(I2S, I2S_MONO_RX_RIGHT);
	I2S_SET_STEREOORDER(I2S, I2S_ORDER_EVENLOW);
	// Set channel width.
	I2S_SET_CHWIDTH(I2S, I2S_CHWIDTH_32);
	// Set FIFO threshold.
	I2S_SET_TXTH(I2S, I2S_FIFO_TX_LEVEL_WORD_8);
	I2S_SET_RXTH(I2S, I2S_FIFO_RX_LEVEL_WORD_8);
	// Enable interrupt.
	I2S_ENABLE_INT(I2S, I2S_RXTH_INT_MASK|I2S_TXTH_INT_MASK);
	// Enable I2S's NVIC. 
...
void I2S0_Slave_Start(void)
{
	...
	if( g_psI2S0_OutBufCtrl != NULL )
	{
		...
		while( !I2S_GET_TX_IS_FULL(I2S) && !BUFCTRL_IS_EMPTY(g_psI2S0_OutBufCtrl) )
		{
...
void I2S_IRQHandler() 
{
	...
	if(I2S_GET_INT_FLAG(I2S, I2S_TXTH_INT_FLAG))
	{
		// Write data process.
		if( g_psI2S0_OutBufCtrl != NULL )
		{
			...
			u32Count = 8; 		
			while( !I2S_GET_TX_IS_FULL(I2S) && u32Count != 0 )
			{
	...
	if(I2S_GET_INT_FLAG(I2S, I2S_RXTH_INT_FLAG))
	{
		// Read data process.
		if( g_psI2S0_InBufCtrl != NULL )
		{
			...
			u32Count = 8; 
			while( !I2S_GET_RX_IS_EMPTY(I2S) && u32Count != 0 )
			{
				// Read the data from I2S RXFIFO.
				u32Data = I2S_READ_RX_FIFO(I2S);
